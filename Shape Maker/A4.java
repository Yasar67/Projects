	...
	* @return toolsPanel		the Panel */
	...
	class MyTreeSelectionListener implements TreeSelectionListener{
		public void valueChanged(TreeSelectionEvent e){
			Shape node = (Shape) tree.getLastSelectedPathComponent();
			if (node instanceof NestedShape){
				TableModelAdapter tableModelAdapter = panel.getTableModelAdapter();
				tableModelAdapter.setNestedShape((NestedShape)node);
			}
		}
	}
		...
		public void actionPerformed(ActionEvent e) {
			if (e.getSource() == addNodeButton) {
				...
				if (selectionPath != null){
					Shape selectedNode = (Shape) selectionPath.getLastPathComponent();
					if (selectedNode instanceof NestedShape){
						panel.addShape(selectionPath);
					}
					else {
						JOptionPane.showMessageDialog(A4.this, "ERROR: Must select a NestedShape node.", "Error Message", JOptionPane.ERROR_MESSAGE);
					}
				}
				else {
					JOptionPane.showMessageDialog(A4.this, "ERROR: No node selected", "Error Message", JOptionPane.ERROR_MESSAGE);
				}
			}
		}
	}
	...
	class RemoveNodeActionListener implements ActionListener{
		public void actionPerformed(ActionEvent e){
			if (e.getSource() == removeNodeButton){
				...
				if (selectionPath != null){
					Shape selectedNode = (Shape) selectionPath.getLastPathComponent();
					if (!(selectedNode.equals(panel.getTreeModelAdapter().getRoot()))){
						panel.removeShape(selectionPath);
					}
					else {
						JOptionPane.showMessageDialog(A4.this, "ERROR: Must not remove the root.", "Error Message", JOptionPane.ERROR_MESSAGE);
					}
				}
				else {
					JOptionPane.showMessageDialog(A4.this, "ERROR: No node selected", "Error Message", JOptionPane.ERROR_MESSAGE);
